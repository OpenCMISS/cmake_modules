...
function(my_stupid_package_dependent_message_function_MagickCore MSG)
    message(STATUS "FindImageMagick wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_MagickCore MSG)
    #message(STATUS "DEBUG FindIMAGEMAGICK wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_MagickCore("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(ImageMagick_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_IMAGEMAGICK flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_IMAGEMAGICK)
     set(OC_SYSTEM_IMAGEMAGICK NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(ImageMagick ${ImageMagick_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (ImageMagick_FOUND)
        set(IMAGEMAGICK_FOUND YES)
        my_stupid_package_dependent_message_function_MagickCore("Found version ${ImageMagick_FIND_VERSION} at ${ImageMagick_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_MagickCore("System search enabled")
    ...
    my_stupid_package_dependent_message_function_MagickCore("Trying to find version ${ImageMagick_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_MagickCore("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${ImageMagick_FIND_REQUIRED})
    UNSET(ImageMagick_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(ImageMagick ${ImageMagick_FIND_VERSION} MODULE QUIET)")
    find_package(ImageMagick ${ImageMagick_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(ImageMagick_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (IMAGEMAGICK_FOUND)
        ...
        set(ImageMagick_FOUND YES)
        if (NOT TARGET MagickCore)
            set(LIBS ${IMAGEMAGICK_LIBRARIES})
            my_stupid_package_dependent_message_function_MagickCore("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED IMAGEMAGICK${DIRSUFF})
                    LIST(APPEND INCS ${IMAGEMAGICK${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_MagickCore("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_MagickCore("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_MagickCore("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(MagickCore UNKNOWN IMPORTED)
            ...
            set_target_properties(MagickCore PROPERTIES 
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_MagickCore("Adding extra library ${LIB} to link interface")
                append_link_library(MagickCore ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_MagickCore("Avoiding double import of target 'MagickCore'")
        endif()
    else()
        my_stupid_package_dependent_message_function_MagickCore("Trying to find version ${ImageMagick_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_MagickCore("Calling find_package(ImageMagick ${IMAGEMAGICK_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(ImageMagick ${IMAGEMAGICK_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (ImageMagick_FOUND)
            set(IMAGEMAGICK_FOUND ${ImageMagick_FOUND})
            my_stupid_package_dependent_message_function_MagickCore("Found at ${ImageMagick_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_MagickCore("No system package found/available.")
            find_package(ImageMagick ${ImageMagick_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (ImageMagick_FIND_REQUIRED AND NOT ImageMagick_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find ImageMagick ${ImageMagick_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system IMAGEMAGICK: ${OC_SYSTEM_IMAGEMAGICK}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_IMAGEMAGICK=YES\n"
