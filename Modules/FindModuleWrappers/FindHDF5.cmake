...
function(my_stupid_package_dependent_message_function_hdf5 MSG)
    message(STATUS "FindHDF5 wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_hdf5 MSG)
    #message(STATUS "DEBUG FindHDF5 wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_hdf5("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(HDF5_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_HDF5 flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_HDF5)
     set(OC_SYSTEM_HDF5 NO) # set it to NO so that we have a value if none is set at all (debug output)
    ...
    if (HDF5_FOUND)
        set(HDF5_FOUND YES)
        my_stupid_package_dependent_message_function_hdf5("Found version ${HDF5_FIND_VERSION} at ${HDF5_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_hdf5("System search enabled")
    ...
    my_stupid_package_dependent_message_function_hdf5("Trying to find version ${HDF5_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_hdf5("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${HDF5_FIND_REQUIRED})
    UNSET(HDF5_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(HDF5 ${HDF5_FIND_VERSION} MODULE QUIET)")
    find_package(HDF5 ${HDF5_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(HDF5_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (HDF5_FOUND)
        ...
        if (NOT TARGET hdf5)
            ...
            my_stupid_package_dependent_message_function_hdf5("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED HDF5${DIRSUFF})
                    LIST(APPEND INCS ${HDF5${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_hdf5("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_hdf5("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_hdf5("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(hdf5 UNKNOWN IMPORTED)
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_hdf5("Adding extra library ${LIB} to link interface")
                append_link_library(hdf5 ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_hdf5("Avoiding double import of target 'hdf5'")
        endif()
    else()
        my_stupid_package_dependent_message_function_hdf5("Trying to find version ${HDF5_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_hdf5("Calling find_package(HDF5 ${HDF5_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(HDF5 ${HDF5_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (HDF5_FOUND)
            set(HDF5_FOUND ${HDF5_FOUND})
            my_stupid_package_dependent_message_function_hdf5("Found at ${HDF5_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_hdf5("No system package found/available.")
            find_package(HDF5 ${HDF5_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (HDF5_FIND_REQUIRED AND NOT HDF5_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find HDF5 ${HDF5_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system HDF5: ${OC_SYSTEM_HDF5}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_HDF5=YES\n"
