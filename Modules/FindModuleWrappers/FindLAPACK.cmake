...
function(my_stupid_package_dependent_message_function_lapack MSG)
    message(STATUS "FindLAPACK wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_lapack MSG)
    #message(STATUS "DEBUG FindLAPACK wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_lapack("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(LAPACK_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_LAPACK flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_LAPACK)
     set(OC_SYSTEM_LAPACK NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(LAPACK ${LAPACK_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (LAPACK_FOUND)
        set(LAPACK_FOUND YES)
        my_stupid_package_dependent_message_function_lapack("Found version ${LAPACK_FIND_VERSION} at ${LAPACK_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_lapack("System search enabled")
    ...
    my_stupid_package_dependent_message_function_lapack("Trying to find version ${LAPACK_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_lapack("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${LAPACK_FIND_REQUIRED})
    UNSET(LAPACK_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(LAPACK ${LAPACK_FIND_VERSION} MODULE QUIET)")
    find_package(LAPACK ${LAPACK_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(LAPACK_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (LAPACK_FOUND)
        ...
        if (NOT TARGET lapack)
            ...
            my_stupid_package_dependent_message_function_lapack("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED LAPACK${DIRSUFF})
                    LIST(APPEND INCS ${LAPACK${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_lapack("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_lapack("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_lapack("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
        ...
        my_stupid_package_dependent_message_function_lapack("Trying to find version ${LAPACK_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_lapack("Calling find_package(LAPACK ${LAPACK_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(LAPACK ${LAPACK_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (LAPACK_FOUND)
            set(LAPACK_FOUND ${LAPACK_FOUND})
            my_stupid_package_dependent_message_function_lapack("Found at ${LAPACK_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_lapack("No system package found/available.")
            find_package(LAPACK ${LAPACK_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (LAPACK_FIND_REQUIRED AND NOT LAPACK_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find LAPACK ${LAPACK_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system LAPACK: ${OC_SYSTEM_LAPACK}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_LAPACK=YES\n"
