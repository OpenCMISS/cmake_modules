...
function(my_stupid_package_dependent_message_function_nglib MSG)
    message(STATUS "FindNETGEN wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_nglib MSG)
    #message(STATUS "DEBUG FindNETGEN wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_nglib("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(NETGEN_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_NETGEN flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_NETGEN)
     set(OC_SYSTEM_NETGEN NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(NETGEN ${NETGEN_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (NETGEN_FOUND)
        set(NETGEN_FOUND YES)
        my_stupid_package_dependent_message_function_nglib("Found version ${NETGEN_FIND_VERSION} at ${NETGEN_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_nglib("System search enabled")
    ...
    my_stupid_package_dependent_message_function_nglib("Trying to find version ${NETGEN_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_nglib("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${NETGEN_FIND_REQUIRED})
    UNSET(NETGEN_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(NETGEN ${NETGEN_FIND_VERSION} MODULE QUIET)")
    find_package(NETGEN ${NETGEN_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(NETGEN_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (NETGEN_FOUND)
        ...
        set(NETGEN_FOUND YES)
        if (NOT TARGET nglib)
            set(LIBS ${NETGEN_LIBRARIES})
            my_stupid_package_dependent_message_function_nglib("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED NETGEN${DIRSUFF})
                    LIST(APPEND INCS ${NETGEN${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_nglib("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_nglib("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_nglib("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(nglib UNKNOWN IMPORTED)
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_nglib("Adding extra library ${LIB} to link interface")
                append_link_library(nglib ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_nglib("Avoiding double import of target 'nglib'")
        endif()
    else()
        my_stupid_package_dependent_message_function_nglib("Trying to find version ${NETGEN_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_nglib("Calling find_package(NETGEN ${NETGEN_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(NETGEN ${NETGEN_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (NETGEN_FOUND)
            set(NETGEN_FOUND ${NETGEN_FOUND})
            my_stupid_package_dependent_message_function_nglib("Found at ${NETGEN_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_nglib("No system package found/available.")
            find_package(NETGEN ${NETGEN_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (NETGEN_FIND_REQUIRED AND NOT NETGEN_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find NETGEN ${NETGEN_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system NETGEN: ${OC_SYSTEM_NETGEN}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_NETGEN=YES\n"
