...
function(my_stupid_package_dependent_message_function_ftgl MSG)
    message(STATUS "FindFTGL wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_ftgl MSG)
    #message(STATUS "DEBUG FindFTGL wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_ftgl("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(FTGL_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_FTGL flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_FTGL)
     set(OC_SYSTEM_FTGL NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(FTGL ${FTGL_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (FTGL_FOUND)
        set(FTGL_FOUND YES)
        my_stupid_package_dependent_message_function_ftgl("Found version ${FTGL_FIND_VERSION} at ${FTGL_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_ftgl("System search enabled")
    ...
    my_stupid_package_dependent_message_function_ftgl("Trying to find version ${FTGL_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_ftgl("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${FTGL_FIND_REQUIRED})
    UNSET(FTGL_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(FTGL ${FTGL_FIND_VERSION} MODULE QUIET)")
    find_package(FTGL ${FTGL_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(FTGL_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (FTGL_FOUND)
        ...
        set(FTGL_FOUND YES)
        if (NOT TARGET ftgl)
            set(LIBS ${FTGL_LIBRARIES})
            my_stupid_package_dependent_message_function_ftgl("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED FTGL${DIRSUFF})
                    LIST(APPEND INCS ${FTGL${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_ftgl("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_ftgl("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_ftgl("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(ftgl UNKNOWN IMPORTED)
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_ftgl("Adding extra library ${LIB} to link interface")
                append_link_library(ftgl ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_ftgl("Avoiding double import of target 'ftgl'")
        endif()
    else()
        my_stupid_package_dependent_message_function_ftgl("Trying to find version ${FTGL_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_ftgl("Calling find_package(FTGL ${FTGL_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(FTGL ${FTGL_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (FTGL_FOUND)
            set(FTGL_FOUND ${FTGL_FOUND})
            my_stupid_package_dependent_message_function_ftgl("Found at ${FTGL_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_ftgl("No system package found/available.")
            find_package(FTGL ${FTGL_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (FTGL_FIND_REQUIRED AND NOT FTGL_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find FTGL ${FTGL_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system FTGL: ${OC_SYSTEM_FTGL}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_FTGL=YES\n"
