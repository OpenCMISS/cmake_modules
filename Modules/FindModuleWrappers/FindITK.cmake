...
function(my_stupid_package_dependent_message_function_itk MSG)
    message(STATUS "FindITK wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_itk MSG)
    #message(STATUS "DEBUG FindITK wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_itk("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(ITK_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_ITK flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_ITK)
     set(OC_SYSTEM_ITK NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(ITK ${ITK_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (ITK_FOUND)
        set(ITK_FOUND YES)
        my_stupid_package_dependent_message_function_itk("Found version ${ITK_FIND_VERSION} at ${ITK_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_itk("System search enabled")
    ...
    my_stupid_package_dependent_message_function_itk("Trying to find version ${ITK_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_itk("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${ITK_FIND_REQUIRED})
    UNSET(ITK_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(ITK ${ITK_FIND_VERSION} MODULE QUIET)")
    find_package(ITK ${ITK_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(ITK_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (ITK_FOUND)
        ...
        if (NOT TARGET itk)
            ...
            my_stupid_package_dependent_message_function_itk("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED ITK${DIRSUFF})
                    LIST(APPEND INCS ${ITK${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_itk("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_itk("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_itk("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(itk UNKNOWN IMPORTED)
            ...
            set_target_properties(itk PROPERTIES 
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_itk("Adding extra library ${LIB} to link interface")
                append_link_library(itk ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_itk("Avoiding double import of target 'itk'")
        endif()
    else()
        my_stupid_package_dependent_message_function_itk("Trying to find version ${ITK_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_itk("Calling find_package(ITK ${ITK_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(ITK ${ITK_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (ITK_FOUND)
            set(ITK_FOUND ${ITK_FOUND})
            my_stupid_package_dependent_message_function_itk("Found at ${ITK_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_itk("No system package found/available.")
            find_package(ITK ${ITK_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (ITK_FIND_REQUIRED AND NOT ITK_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find ITK ${ITK_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system ITK: ${OC_SYSTEM_ITK}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_ITK=YES\n"
