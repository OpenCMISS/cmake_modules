...
function(my_stupid_package_dependent_message_function_parmetis MSG)
    message(STATUS "FindPARMETIS wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_parmetis MSG)
    #message(STATUS "DEBUG FindPARMETIS wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_parmetis("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(PARMETIS_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_PARMETIS flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_PARMETIS)
     set(OC_SYSTEM_PARMETIS NO) # set it to NO so that we have a value if none is set at all (debug output)
    ...
    if (PARMETIS_FOUND)
        ...
        my_stupid_package_dependent_message_function_parmetis("Found version ${PARMETIS_FIND_VERSION} at ${PARMETIS_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_parmetis("System search enabled")
    ...
    my_stupid_package_dependent_message_function_parmetis("Trying to find version ${PARMETIS_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_parmetis("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${PARMETIS_FIND_REQUIRED})
    UNSET(PARMETIS_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(PARMETIS ${PARMETIS_FIND_VERSION} MODULE QUIET)")
    find_package(PARMETIS ${PARMETIS_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(PARMETIS_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (PARMETIS_FOUND)
        ...
        if (NOT TARGET parmetis)
            set(LIBS ${PARMETIS_LIBRARIES})
            my_stupid_package_dependent_message_function_parmetis("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED PARMETIS${DIRSUFF})
                    LIST(APPEND INCS ${PARMETIS${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_parmetis("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_parmetis("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_parmetis("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(parmetis UNKNOWN IMPORTED)
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_parmetis("Adding extra library ${LIB} to link interface")
                append_link_library(parmetis ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_parmetis("Avoiding double import of target 'parmetis'")
        endif()
    else()
        my_stupid_package_dependent_message_function_parmetis("Trying to find version ${PARMETIS_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_parmetis("Calling find_package(PARMETIS ${PARMETIS_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(PARMETIS ${PARMETIS_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (PARMETIS_FOUND)
            set(PARMETIS_FOUND ${PARMETIS_FOUND})
            my_stupid_package_dependent_message_function_parmetis("Found at ${PARMETIS_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_parmetis("No system package found/available.")
            find_package(PARMETIS ${PARMETIS_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (PARMETIS_FIND_REQUIRED AND NOT PARMETIS_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find PARMETIS ${PARMETIS_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system PARMETIS: ${OC_SYSTEM_PARMETIS}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_PARMETIS=YES\n"
