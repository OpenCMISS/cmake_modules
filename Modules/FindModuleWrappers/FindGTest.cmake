...
function(my_stupid_package_dependent_message_function_gtest_main MSG)
    message(STATUS "FindGTest wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_gtest_main MSG)
    #message(STATUS "DEBUG FindGTEST wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_gtest_main("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(GTest_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_GTEST flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_GTEST)
     set(OC_SYSTEM_GTEST NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(GTest ${GTest_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (GTest_FOUND)
        set(GTEST_FOUND YES)
        my_stupid_package_dependent_message_function_gtest_main("Found version ${GTest_FIND_VERSION} at ${GTest_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_gtest_main("System search enabled")
    ...
    my_stupid_package_dependent_message_function_gtest_main("Trying to find version ${GTest_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_gtest_main("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${GTest_FIND_REQUIRED})
    UNSET(GTest_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(GTest ${GTest_FIND_VERSION} MODULE QUIET)")
    find_package(GTest ${GTest_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(GTest_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (GTEST_FOUND)
        ...
        if (NOT TARGET gtest_main)
            ...
            my_stupid_package_dependent_message_function_gtest_main("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED GTEST${DIRSUFF})
                    LIST(APPEND INCS ${GTEST${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_gtest_main("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_gtest_main("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_gtest_main("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_gtest_main("Adding extra library ${LIB} to link interface")
                append_link_library(gtest_main ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_gtest_main("Avoiding double import of target 'gtest_main'")
        endif()
    else()
        my_stupid_package_dependent_message_function_gtest_main("Trying to find version ${GTest_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_gtest_main("Calling find_package(GTest ${GTEST_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(GTest ${GTEST_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (GTest_FOUND)
            set(GTEST_FOUND ${GTest_FOUND})
            my_stupid_package_dependent_message_function_gtest_main("Found at ${GTest_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_gtest_main("No system package found/available.")
            find_package(GTest ${GTest_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (GTest_FIND_REQUIRED AND NOT GTest_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find GTest ${GTest_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system GTEST: ${OC_SYSTEM_GTEST}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_GTEST=YES\n"
