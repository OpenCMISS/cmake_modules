...
function(my_stupid_package_dependent_message_function_png MSG)
    message(STATUS "FindPNG wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_png MSG)
    #message(STATUS "DEBUG FindPNG wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_png("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(PNG_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_PNG flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_PNG)
     set(OC_SYSTEM_PNG NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(PNG ${PNG_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (PNG_FOUND)
        set(PNG_FOUND YES)
        my_stupid_package_dependent_message_function_png("Found version ${PNG_FIND_VERSION} at ${PNG_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_png("System search enabled")
    ...
    my_stupid_package_dependent_message_function_png("Trying to find version ${PNG_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_png("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${PNG_FIND_REQUIRED})
    UNSET(PNG_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(PNG ${PNG_FIND_VERSION} MODULE QUIET)")
    find_package(PNG ${PNG_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(PNG_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (PNG_FOUND)
        ...
        if (NOT TARGET png)
            ...
            my_stupid_package_dependent_message_function_png("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED PNG${DIRSUFF})
                    LIST(APPEND INCS ${PNG${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_png("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_png("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_png("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_png("Adding extra library ${LIB} to link interface")
                append_link_library(png ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_png("Avoiding double import of target 'png'")
        endif()
    else()
        my_stupid_package_dependent_message_function_png("Trying to find version ${PNG_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_png("Calling find_package(PNG ${PNG_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(PNG ${PNG_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (PNG_FOUND)
            set(PNG_FOUND ${PNG_FOUND})
            my_stupid_package_dependent_message_function_png("Found at ${PNG_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_png("No system package found/available.")
            find_package(PNG ${PNG_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (PNG_FIND_REQUIRED AND NOT PNG_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find PNG ${PNG_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system PNG: ${OC_SYSTEM_PNG}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_PNG=YES\n"
