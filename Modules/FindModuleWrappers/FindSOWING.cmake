...
function(my_stupid_package_dependent_message_function_sowing MSG)
    message(STATUS "FindSOWING wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_sowing MSG)
    #message(STATUS "DEBUG FindSOWING wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_sowing("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(SOWING_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_SOWING flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_SOWING)
     set(OC_SYSTEM_SOWING NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(SOWING ${SOWING_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (SOWING_FOUND)
        set(SOWING_FOUND YES)
        my_stupid_package_dependent_message_function_sowing("Found version ${SOWING_FIND_VERSION} at ${SOWING_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_sowing("System search enabled")
    ...
    my_stupid_package_dependent_message_function_sowing("Trying to find version ${SOWING_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_sowing("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${SOWING_FIND_REQUIRED})
    UNSET(SOWING_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(SOWING ${SOWING_FIND_VERSION} MODULE QUIET)")
    find_package(SOWING ${SOWING_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(SOWING_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (SOWING_FOUND)
        ...
        set(SOWING_FOUND YES)
        if (NOT TARGET sowing)
            set(LIBS ${SOWING_LIBRARIES})
            my_stupid_package_dependent_message_function_sowing("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED SOWING${DIRSUFF})
                    LIST(APPEND INCS ${SOWING${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_sowing("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_sowing("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_sowing("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(sowing UNKNOWN IMPORTED)
            ...
            set_target_properties(sowing PROPERTIES 
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_sowing("Adding extra library ${LIB} to link interface")
                append_link_library(sowing ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_sowing("Avoiding double import of target 'sowing'")
        endif()
    else()
        my_stupid_package_dependent_message_function_sowing("Trying to find version ${SOWING_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_sowing("Calling find_package(SOWING ${SOWING_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(SOWING ${SOWING_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (SOWING_FOUND)
            set(SOWING_FOUND ${SOWING_FOUND})
            my_stupid_package_dependent_message_function_sowing("Found at ${SOWING_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_sowing("No system package found/available.")
            find_package(SOWING ${SOWING_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (SOWING_FIND_REQUIRED AND NOT SOWING_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find SOWING ${SOWING_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system SOWING: ${OC_SYSTEM_SOWING}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_SOWING=YES\n"
