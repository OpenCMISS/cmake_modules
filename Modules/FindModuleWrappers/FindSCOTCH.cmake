...
if (NOT OC_SYSTEM_SCOTCH)
     set(OC_SYSTEM_SCOTCH NO) # set it to NO so that we have a value if none is set at all (debug output)
    ...
    if (SCOTCH_FOUND)
        ...
        my_stupid_package_dependent_message_function_scotch("Found version ${SCOTCH_FIND_VERSION} at ${SCOTCH_DIR} in CONFIG mode")
    endif()
else()
        ...
        if (NOT TARGET scotch)
            ...
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED SCOTCH${DIRSUFF})
                    LIST(APPEND INCS ${SCOTCH${DIRSUFF}})
                endif()
            endforeach()
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_scotch("Adding extra library ${LIB} to link interface")
                append_link_library(scotch ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_scotch("Avoiding double import of target 'scotch'")
        endif()
    else()
        my_stupid_package_dependent_message_function_scotch("Trying to find version ${SCOTCH_FIND_VERSION} on system in CONFIG mode")
...
if (SCOTCH_FIND_REQUIRED AND NOT SCOTCH_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        ...
        "Allow system SCOTCH: ${OC_SYSTEM_SCOTCH}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_SCOTCH=YES\n"
