...
function(my_stupid_package_dependent_message_function_csim MSG)
    message(STATUS "FindCSim wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_csim MSG)
    #message(STATUS "DEBUG FindCSIM wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_csim("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(CSim_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_CSIM flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_CSIM)
     set(OC_SYSTEM_CSIM NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(CSim ${CSim_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (CSim_FOUND)
        set(CSIM_FOUND YES)
        my_stupid_package_dependent_message_function_csim("Found version ${CSim_FIND_VERSION} at ${CSim_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_csim("System search enabled")
    ...
    my_stupid_package_dependent_message_function_csim("Trying to find version ${CSim_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_csim("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${CSim_FIND_REQUIRED})
    UNSET(CSim_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(CSim ${CSim_FIND_VERSION} MODULE QUIET)")
    find_package(CSim ${CSim_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(CSim_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (CSIM_FOUND)
        ...
        set(CSim_FOUND YES)
        if (NOT TARGET csim)
            set(LIBS ${CSIM_LIBRARIES})
            my_stupid_package_dependent_message_function_csim("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED CSIM${DIRSUFF})
                    LIST(APPEND INCS ${CSIM${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_csim("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_csim("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_csim("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(csim UNKNOWN IMPORTED)
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_csim("Adding extra library ${LIB} to link interface")
                append_link_library(csim ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_csim("Avoiding double import of target 'csim'")
        endif()
    else()
        my_stupid_package_dependent_message_function_csim("Trying to find version ${CSim_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_csim("Calling find_package(CSim ${CSIM_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(CSim ${CSIM_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (CSim_FOUND)
            set(CSIM_FOUND ${CSim_FOUND})
            my_stupid_package_dependent_message_function_csim("Found at ${CSim_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_csim("No system package found/available.")
            find_package(CSim ${CSim_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (CSim_FIND_REQUIRED AND NOT CSim_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find CSim ${CSim_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system CSIM: ${OC_SYSTEM_CSIM}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_CSIM=YES\n"
