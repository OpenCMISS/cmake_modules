...
function(my_stupid_package_dependent_message_function_optpp MSG)
    message(STATUS "FindOPTPP wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_optpp MSG)
    #message(STATUS "DEBUG FindOPTPP wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_optpp("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(OPTPP_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_OPTPP flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_OPTPP)
     set(OC_SYSTEM_OPTPP NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(OPTPP ${OPTPP_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (OPTPP_FOUND)
        set(OPTPP_FOUND YES)
        my_stupid_package_dependent_message_function_optpp("Found version ${OPTPP_FIND_VERSION} at ${OPTPP_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_optpp("System search enabled")
    ...
    my_stupid_package_dependent_message_function_optpp("Trying to find version ${OPTPP_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_optpp("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${OPTPP_FIND_REQUIRED})
    UNSET(OPTPP_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(OPTPP ${OPTPP_FIND_VERSION} MODULE QUIET)")
    find_package(OPTPP ${OPTPP_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(OPTPP_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (OPTPP_FOUND)
        ...
        set(OPTPP_FOUND YES)
        if (NOT TARGET optpp)
            set(LIBS ${OPTPP_LIBRARIES})
            my_stupid_package_dependent_message_function_optpp("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED OPTPP${DIRSUFF})
                    LIST(APPEND INCS ${OPTPP${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_optpp("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_optpp("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_optpp("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(optpp UNKNOWN IMPORTED)
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_optpp("Adding extra library ${LIB} to link interface")
                append_link_library(optpp ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_optpp("Avoiding double import of target 'optpp'")
        endif()
    else()
        my_stupid_package_dependent_message_function_optpp("Trying to find version ${OPTPP_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_optpp("Calling find_package(OPTPP ${OPTPP_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(OPTPP ${OPTPP_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (OPTPP_FOUND)
            set(OPTPP_FOUND ${OPTPP_FOUND})
            my_stupid_package_dependent_message_function_optpp("Found at ${OPTPP_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_optpp("No system package found/available.")
            find_package(OPTPP ${OPTPP_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (OPTPP_FIND_REQUIRED AND NOT OPTPP_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find OPTPP ${OPTPP_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system OPTPP: ${OC_SYSTEM_OPTPP}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_OPTPP=YES\n"
