...
function(my_stupid_package_dependent_message_function_zlib MSG)
    message(STATUS "FindZLIB wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_zlib MSG)
    #message(STATUS "DEBUG FindZLIB wrapper: ${MSG}")
endfunction()
...
if (ZLIB_FIND_SYSTEM)
    ...
    my_stupid_package_dependent_message_function_zlib("Trying to find version ${ZLIB_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_zlib("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${ZLIB_FIND_REQUIRED})
    ...
    find_package(ZLIB ${ZLIB_FIND_VERSION} MODULE QUIET)
    ...
    if (ZLIB_FOUND)
        ...
        if (NOT TARGET zlib)
            ...
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED ZLIB${DIRSUFF})
                    LIST(APPEND INCS ${ZLIB${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_zlib("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_zlib("Converting found module to imported targets")
            ...
            if (_OPTIMIZED_CONFIG_INDEX EQUAL -1 AND _DEBUG_CONFIG_INDEX EQUAL -1)
                my_stupid_package_dependent_message_function_debug_zlib("Index: _OPTIMIZED_CONFIG_INDEX=${_OPTIMIZED_CONFIG_INDEX}, _DEBUG_CONFIG_INDEX=${_DEBUG_CONFIG_INDEX}")
                add_configuration_link_libraries(zlib ${CURRENT_BUILD_TYPE} "${LIBS}")
            else()
                my_stupid_package_dependent_message_function_debug_zlib("Index: _OPTIMIZED_CONFIG_INDEX=${_OPTIMIZED_CONFIG_INDEX}, _DEBUG_CONFIG_INDEX=${_DEBUG_CONFIG_INDEX}")
                ...
                if (_OPTIMIZED_CONFIG_INDEX GREATER -1)
                    ...
                    add_configuration_link_libraries(zlib RELEASE "${_EXTRACTED_LIBS}")
                endif()
                if (_DEBUG_CONFIG_INDEX GREATER -1)
                    extract_config_libs(${_DEBUG_CONFIG_INDEX} "${LIBS}" _EXTRACTED_LIBS)
                    add_configuration_link_libraries(zlib DEBUG "${_EXTRACTED_LIBS}")
                endif()
            endif()
        else()
