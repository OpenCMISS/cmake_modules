...
function(my_stupid_package_dependent_message_function_slepc MSG)
    message(STATUS "FindSLEPC wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_slepc MSG)
    #message(STATUS "DEBUG FindSLEPC wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_slepc("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(SLEPC_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_SLEPC flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_SLEPC)
     set(OC_SYSTEM_SLEPC NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(SLEPC ${SLEPC_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (SLEPC_FOUND)
        set(SLEPC_FOUND YES)
        my_stupid_package_dependent_message_function_slepc("Found version ${SLEPC_FIND_VERSION} at ${SLEPC_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_slepc("System search enabled")
    ...
    my_stupid_package_dependent_message_function_slepc("Trying to find version ${SLEPC_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_slepc("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${SLEPC_FIND_REQUIRED})
    UNSET(SLEPC_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(SLEPC ${SLEPC_FIND_VERSION} MODULE QUIET)")
    find_package(SLEPC ${SLEPC_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(SLEPC_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (SLEPC_FOUND)
        ...
        set(SLEPC_FOUND YES)
        if (NOT TARGET slepc)
            set(LIBS ${SLEPC_LIBRARIES})
            my_stupid_package_dependent_message_function_slepc("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED SLEPC${DIRSUFF})
                    LIST(APPEND INCS ${SLEPC${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_slepc("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_slepc("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_slepc("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(slepc UNKNOWN IMPORTED)
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_slepc("Adding extra library ${LIB} to link interface")
                append_link_library(slepc ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_slepc("Avoiding double import of target 'slepc'")
        endif()
    else()
        my_stupid_package_dependent_message_function_slepc("Trying to find version ${SLEPC_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_slepc("Calling find_package(SLEPC ${SLEPC_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(SLEPC ${SLEPC_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (SLEPC_FOUND)
            set(SLEPC_FOUND ${SLEPC_FOUND})
            my_stupid_package_dependent_message_function_slepc("Found at ${SLEPC_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_slepc("No system package found/available.")
            find_package(SLEPC ${SLEPC_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (SLEPC_FIND_REQUIRED AND NOT SLEPC_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find SLEPC ${SLEPC_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system SLEPC: ${OC_SYSTEM_SLEPC}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_SLEPC=YES\n"
