...
function(my_stupid_package_dependent_message_function_suitesparse MSG)
    message(STATUS "FindSUITESPARSE wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_suitesparse MSG)
    #message(STATUS "DEBUG FindSUITESPARSE wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_suitesparse("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(SUITESPARSE_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_SUITESPARSE flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_SUITESPARSE)
     set(OC_SYSTEM_SUITESPARSE NO) # set it to NO so that we have a value if none is set at all (debug output)
    ...
    if (SUITESPARSE_FOUND)
        ...
        my_stupid_package_dependent_message_function_suitesparse("Found version ${SUITESPARSE_FIND_VERSION} at ${SUITESPARSE_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_suitesparse("System search enabled")
    ...
    my_stupid_package_dependent_message_function_suitesparse("Trying to find version ${SUITESPARSE_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_suitesparse("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${SUITESPARSE_FIND_REQUIRED})
    UNSET(SUITESPARSE_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(SUITESPARSE ${SUITESPARSE_FIND_VERSION} MODULE QUIET)")
    find_package(SUITESPARSE ${SUITESPARSE_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(SUITESPARSE_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (SUITESPARSE_FOUND)
        ...
        if (NOT TARGET suitesparse)
            set(LIBS ${SUITESPARSE_LIBRARIES})
            my_stupid_package_dependent_message_function_suitesparse("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED SUITESPARSE${DIRSUFF})
                    LIST(APPEND INCS ${SUITESPARSE${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_suitesparse("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_suitesparse("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_suitesparse("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_suitesparse("Adding extra library ${LIB} to link interface")
                append_link_library(suitesparse ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_suitesparse("Avoiding double import of target 'suitesparse'")
        endif()
    else()
        my_stupid_package_dependent_message_function_suitesparse("Trying to find version ${SUITESPARSE_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_suitesparse("Calling find_package(SUITESPARSE ${SUITESPARSE_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(SUITESPARSE ${SUITESPARSE_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (SUITESPARSE_FOUND)
            set(SUITESPARSE_FOUND ${SUITESPARSE_FOUND})
            my_stupid_package_dependent_message_function_suitesparse("Found at ${SUITESPARSE_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_suitesparse("No system package found/available.")
            find_package(SUITESPARSE ${SUITESPARSE_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (SUITESPARSE_FIND_REQUIRED AND NOT SUITESPARSE_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find SUITESPARSE ${SUITESPARSE_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system SUITESPARSE: ${OC_SYSTEM_SUITESPARSE}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_SUITESPARSE=YES\n"
