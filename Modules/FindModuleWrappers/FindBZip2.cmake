...
function(my_stupid_package_dependent_message_function_bz2 MSG)
    message(STATUS "FindBZip2 wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_bz2 MSG)
    #message(STATUS "DEBUG FindBZIP2 wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_bz2("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(BZip2_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_BZIP2 flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_BZIP2)
     set(OC_SYSTEM_BZIP2 NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(BZip2 ${BZip2_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (BZip2_FOUND)
        set(BZIP2_FOUND YES)
        my_stupid_package_dependent_message_function_bz2("Found version ${BZip2_FIND_VERSION} at ${BZip2_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_bz2("System search enabled")
    ...
    my_stupid_package_dependent_message_function_bz2("Trying to find version ${BZip2_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_bz2("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${BZip2_FIND_REQUIRED})
    UNSET(BZip2_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(BZip2 ${BZip2_FIND_VERSION} MODULE QUIET)")
    find_package(BZip2 ${BZip2_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(BZip2_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (BZIP2_FOUND)
        ...
        if (NOT TARGET bz2)
            ...
            my_stupid_package_dependent_message_function_bz2("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED BZIP2${DIRSUFF})
                    LIST(APPEND INCS ${BZIP2${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_bz2("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_bz2("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_bz2("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_bz2("Adding extra library ${LIB} to link interface")
                append_link_library(bz2 ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_bz2("Avoiding double import of target 'bz2'")
        endif()
    else()
        my_stupid_package_dependent_message_function_bz2("Trying to find version ${BZip2_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_bz2("Calling find_package(BZip2 ${BZIP2_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(BZip2 ${BZIP2_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (BZip2_FOUND)
            set(BZIP2_FOUND ${BZip2_FOUND})
            my_stupid_package_dependent_message_function_bz2("Found at ${BZip2_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_bz2("No system package found/available.")
            find_package(BZip2 ${BZip2_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (BZip2_FIND_REQUIRED AND NOT BZip2_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find BZip2 ${BZip2_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system BZIP2: ${OC_SYSTEM_BZIP2}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_BZIP2=YES\n"
