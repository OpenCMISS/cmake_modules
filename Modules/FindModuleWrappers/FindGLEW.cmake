...
function(my_stupid_package_dependent_message_function_glew MSG)
    message(STATUS "FindGLEW wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_glew MSG)
    #message(STATUS "DEBUG FindGLEW wrapper: ${MSG}")
endfunction()
...
if (GLEW_FIND_SYSTEM)
    ...
    my_stupid_package_dependent_message_function_glew("Trying to find version ${GLEW_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_glew("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${GLEW_FIND_REQUIRED})
    UNSET(GLEW_FIND_REQUIRED)
    ...
    find_package(GLEW ${GLEW_FIND_VERSION} MODULE QUIET)
    ...
    if (GLEW_FOUND)
        ...
        if (NOT TARGET glew)
            ...
            my_stupid_package_dependent_message_function_glew("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED GLEW${DIRSUFF})
                    LIST(APPEND INCS ${GLEW${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_glew("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_glew("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_glew("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            ...
            if (_OPTIMIZED_CONFIG_INDEX EQUAL -1 AND _DEBUG_CONFIG_INDEX EQUAL -1)
                my_stupid_package_dependent_message_function_debug_glew("Index: _OPTIMIZED_CONFIG_INDEX=${_OPTIMIZED_CONFIG_INDEX}, _DEBUG_CONFIG_INDEX=${_DEBUG_CONFIG_INDEX}")
                add_configuration_link_libraries(glew ${CURRENT_BUILD_TYPE} "${LIBS}")
            else()
                my_stupid_package_dependent_message_function_debug_glew("Index: _OPTIMIZED_CONFIG_INDEX=${_OPTIMIZED_CONFIG_INDEX}, _DEBUG_CONFIG_INDEX=${_DEBUG_CONFIG_INDEX}")
                ...
                if (_OPTIMIZED_CONFIG_INDEX GREATER -1)
                    ...
                    add_configuration_link_libraries(glew RELEASE "${_EXTRACTED_LIBS}")
                endif()
                if (_DEBUG_CONFIG_INDEX GREATER -1)
                    extract_config_libs(${_DEBUG_CONFIG_INDEX} "${LIBS}" _EXTRACTED_LIBS)
                    add_configuration_link_libraries(glew DEBUG "${_EXTRACTED_LIBS}")
                endif()
            endif()
        else()
