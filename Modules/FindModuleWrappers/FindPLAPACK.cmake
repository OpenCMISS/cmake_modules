...
function(my_stupid_package_dependent_message_function_plapack MSG)
    message(STATUS "FindPLAPACK wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_plapack MSG)
    #message(STATUS "DEBUG FindPLAPACK wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_plapack("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(PLAPACK_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_PLAPACK flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_PLAPACK)
     set(OC_SYSTEM_PLAPACK NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(PLAPACK ${PLAPACK_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (PLAPACK_FOUND)
        set(PLAPACK_FOUND YES)
        my_stupid_package_dependent_message_function_plapack("Found version ${PLAPACK_FIND_VERSION} at ${PLAPACK_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_plapack("System search enabled")
    ...
    my_stupid_package_dependent_message_function_plapack("Trying to find version ${PLAPACK_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_plapack("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${PLAPACK_FIND_REQUIRED})
    UNSET(PLAPACK_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(PLAPACK ${PLAPACK_FIND_VERSION} MODULE QUIET)")
    find_package(PLAPACK ${PLAPACK_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(PLAPACK_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (PLAPACK_FOUND)
        ...
        set(PLAPACK_FOUND YES)
        if (NOT TARGET plapack)
            set(LIBS ${PLAPACK_LIBRARIES})
            my_stupid_package_dependent_message_function_plapack("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED PLAPACK${DIRSUFF})
                    LIST(APPEND INCS ${PLAPACK${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_plapack("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_plapack("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_plapack("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(plapack UNKNOWN IMPORTED)
            ...
            set_target_properties(plapack PROPERTIES 
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_plapack("Adding extra library ${LIB} to link interface")
                append_link_library(plapack ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_plapack("Avoiding double import of target 'plapack'")
        endif()
    else()
        my_stupid_package_dependent_message_function_plapack("Trying to find version ${PLAPACK_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_plapack("Calling find_package(PLAPACK ${PLAPACK_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(PLAPACK ${PLAPACK_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (PLAPACK_FOUND)
            set(PLAPACK_FOUND ${PLAPACK_FOUND})
            my_stupid_package_dependent_message_function_plapack("Found at ${PLAPACK_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_plapack("No system package found/available.")
            find_package(PLAPACK ${PLAPACK_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (PLAPACK_FIND_REQUIRED AND NOT PLAPACK_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find PLAPACK ${PLAPACK_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system PLAPACK: ${OC_SYSTEM_PLAPACK}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_PLAPACK=YES\n"
