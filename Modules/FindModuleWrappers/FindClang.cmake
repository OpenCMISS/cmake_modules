    ...
    if (CLANG_FOUND)
        ...
        if (NOT TARGET clang)
            ...
            if (_OPTIMIZED_CONFIG_INDEX EQUAL -1 AND _DEBUG_CONFIG_INDEX EQUAL -1)
                my_stupid_package_dependent_message_function_debug_clang("Index: _OPTIMIZED_CONFIG_INDEX=${_OPTIMIZED_CONFIG_INDEX}, _DEBUG_CONFIG_INDEX=${_DEBUG_CONFIG_INDEX}")
                add_configuration_link_libraries(clang ${CURRENT_BUILD_TYPE} "${LIBS}")
            else()
                my_stupid_package_dependent_message_function_debug_clang("Index: _OPTIMIZED_CONFIG_INDEX=${_OPTIMIZED_CONFIG_INDEX}, _DEBUG_CONFIG_INDEX=${_DEBUG_CONFIG_INDEX}")
                ...
                if (_OPTIMIZED_CONFIG_INDEX GREATER -1)
                    ...
                    add_configuration_link_libraries(clang RELEASE "${_EXTRACTED_LIBS}")
                endif()
                if (_DEBUG_CONFIG_INDEX GREATER -1)
                    extract_config_libs(${_DEBUG_CONFIG_INDEX} "${LIBS}" _EXTRACTED_LIBS)
                    add_configuration_link_libraries(clang DEBUG "${_EXTRACTED_LIBS}")
                endif()
            endif()
        else()
...
if (Clang_FIND_REQUIRED AND NOT Clang_FOUND)
    message(FATAL_ERROR "FindModuleWrapper error!\n"
        ...
        "Allow system search for CLANG: ${CLANG_FIND_SYSTEM}\n"
