...
function(my_stupid_package_dependent_message_function_clang MSG)
    message(STATUS "FindClang wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_clang MSG)
    #message(STATUS "DEBUG FindCLANG wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_clang("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(Clang_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_CLANG flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_CLANG)
     set(OC_SYSTEM_CLANG NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(Clang ${Clang_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (Clang_FOUND)
        set(CLANG_FOUND YES)
        my_stupid_package_dependent_message_function_clang("Found version ${Clang_FIND_VERSION} at ${Clang_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_clang("System search enabled")
    ...
    my_stupid_package_dependent_message_function_clang("Trying to find version ${Clang_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_clang("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${Clang_FIND_REQUIRED})
    UNSET(Clang_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(Clang ${Clang_FIND_VERSION} MODULE QUIET)")
    find_package(Clang ${Clang_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(Clang_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (CLANG_FOUND)
        ...
        set(Clang_FOUND YES)
        if (NOT TARGET clang)
            set(LIBS ${CLANG_LIBRARIES})
            my_stupid_package_dependent_message_function_clang("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED CLANG${DIRSUFF})
                    LIST(APPEND INCS ${CLANG${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_clang("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_clang("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_clang("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_clang("Adding extra library ${LIB} to link interface")
                append_link_library(clang ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_clang("Avoiding double import of target 'clang'")
        endif()
    else()
        my_stupid_package_dependent_message_function_clang("Trying to find version ${Clang_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_clang("Calling find_package(Clang ${CLANG_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(Clang ${CLANG_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (Clang_FOUND)
            set(CLANG_FOUND ${Clang_FOUND})
            my_stupid_package_dependent_message_function_clang("Found at ${Clang_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_clang("No system package found/available.")
            find_package(Clang ${Clang_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (Clang_FIND_REQUIRED AND NOT Clang_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find Clang ${Clang_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system CLANG: ${OC_SYSTEM_CLANG}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_CLANG=YES\n"
