...
function(my_stupid_package_dependent_message_function_blas MSG)
    message(STATUS "FindBLAS wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_blas MSG)
    #message(STATUS "DEBUG FindBLAS wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_blas("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(BLAS_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_BLAS flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_BLAS)
     set(OC_SYSTEM_BLAS NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(BLAS ${BLAS_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (BLAS_FOUND)
        set(BLAS_FOUND YES)
        my_stupid_package_dependent_message_function_blas("Found version ${BLAS_FIND_VERSION} at ${BLAS_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_blas("System search enabled")
    ...
    my_stupid_package_dependent_message_function_blas("Trying to find version ${BLAS_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_blas("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${BLAS_FIND_REQUIRED})
    UNSET(BLAS_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(BLAS ${BLAS_FIND_VERSION} MODULE QUIET)")
    find_package(BLAS ${BLAS_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(BLAS_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (BLAS_FOUND)
        ...
        if (NOT TARGET blas)
            ...
            my_stupid_package_dependent_message_function_blas("Found: ${LIBS}")
            ...
            endforeach()
            my_stupid_package_dependent_message_function_debug_blas("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_blas("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_blas("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_blas("Adding extra library ${LIB} to link interface")
                append_link_library(blas ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_blas("Avoiding double import of target 'blas'")
        endif()
    else()
        my_stupid_package_dependent_message_function_blas("Trying to find version ${BLAS_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_blas("Calling find_package(BLAS ${BLAS_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(BLAS ${BLAS_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (BLAS_FOUND)
            set(BLAS_FOUND ${BLAS_FOUND})
            my_stupid_package_dependent_message_function_blas("Found at ${BLAS_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_blas("No system package found/available.")
            find_package(BLAS ${BLAS_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (BLAS_FIND_REQUIRED AND NOT BLAS_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find BLAS ${BLAS_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system BLAS: ${OC_SYSTEM_BLAS}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_BLAS=YES\n"
