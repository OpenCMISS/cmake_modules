...
function(my_stupid_package_dependent_message_function_freetype MSG)
    message(STATUS "FindFreetype wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_freetype MSG)
    #message(STATUS "DEBUG FindFREETYPE wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_freetype("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(Freetype_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_FREETYPE flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_FREETYPE)
     set(OC_SYSTEM_FREETYPE NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(Freetype ${Freetype_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (Freetype_FOUND)
        set(FREETYPE_FOUND YES)
        my_stupid_package_dependent_message_function_freetype("Found version ${Freetype_FIND_VERSION} at ${Freetype_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_freetype("System search enabled")
    ...
    my_stupid_package_dependent_message_function_freetype("Trying to find version ${Freetype_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_freetype("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${Freetype_FIND_REQUIRED})
    UNSET(Freetype_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(Freetype ${Freetype_FIND_VERSION} MODULE QUIET)")
    find_package(Freetype ${Freetype_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(Freetype_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (FREETYPE_FOUND)
        ...
        if (NOT TARGET freetype)
            set(LIBS ${FREETYPE_LIBRARIES})
            my_stupid_package_dependent_message_function_freetype("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED FREETYPE${DIRSUFF})
                    LIST(APPEND INCS ${FREETYPE${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_freetype("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_freetype("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_freetype("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_freetype("Adding extra library ${LIB} to link interface")
                append_link_library(freetype ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_freetype("Avoiding double import of target 'freetype'")
        endif()
    else()
        my_stupid_package_dependent_message_function_freetype("Trying to find version ${Freetype_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_freetype("Calling find_package(Freetype ${FREETYPE_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(Freetype ${FREETYPE_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (Freetype_FOUND)
            set(FREETYPE_FOUND ${Freetype_FOUND})
            my_stupid_package_dependent_message_function_freetype("Found at ${Freetype_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_freetype("No system package found/available.")
            find_package(Freetype ${Freetype_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (Freetype_FIND_REQUIRED AND NOT Freetype_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find Freetype ${Freetype_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system FREETYPE: ${OC_SYSTEM_FREETYPE}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_FREETYPE=YES\n"
