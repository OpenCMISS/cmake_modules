    ...
    if (PETSC_FOUND)
        ...
        if (NOT TARGET petsc)
            ...
            if (_OPTIMIZED_CONFIG_INDEX EQUAL -1 AND _DEBUG_CONFIG_INDEX EQUAL -1)
                my_stupid_package_dependent_message_function_debug_petsc("Index: _OPTIMIZED_CONFIG_INDEX=${_OPTIMIZED_CONFIG_INDEX}, _DEBUG_CONFIG_INDEX=${_DEBUG_CONFIG_INDEX}")
                add_configuration_link_libraries(petsc ${CURRENT_BUILD_TYPE} "${LIBS}")
            else()
                my_stupid_package_dependent_message_function_debug_petsc("Index: _OPTIMIZED_CONFIG_INDEX=${_OPTIMIZED_CONFIG_INDEX}, _DEBUG_CONFIG_INDEX=${_DEBUG_CONFIG_INDEX}")
                ...
                if (_OPTIMIZED_CONFIG_INDEX GREATER -1)
                    ...
                    add_configuration_link_libraries(petsc RELEASE "${_EXTRACTED_LIBS}")
                endif()
                if (_DEBUG_CONFIG_INDEX GREATER -1)
                    extract_config_libs(${_DEBUG_CONFIG_INDEX} "${LIBS}" _EXTRACTED_LIBS)
                    add_configuration_link_libraries(petsc DEBUG "${_EXTRACTED_LIBS}")
                endif()
            endif()
        else()
...
if (PETSC_FIND_REQUIRED AND NOT PETSC_FOUND)
    message(FATAL_ERROR "FindModuleWrapper error!\n"
        ...
        "Allow system search for PETSC: ${PETSC_FIND_SYSTEM}\n"
