...
function(my_stupid_package_dependent_message_function_pastix MSG)
    message(STATUS "FindPASTIX wrapper: ${MSG}")
endfunction()
function(my_stupid_package_dependent_message_function_debug_pastix MSG)
    #message(STATUS "DEBUG FindPASTIX wrapper: ${MSG}")
endfunction()

my_stupid_package_dependent_message_function_debug_pastix("Entering script. CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}, _IMPORT_PREFIX=${_IMPORT_PREFIX}")

# Default: Not found
SET(PASTIX_FOUND NO)
    
# The default way is to look for components in the current PREFIX_PATH, e.g. own build components.
# If the OC_SYSTEM_PASTIX flag is set for a package, the MODULE and CONFIG modes are tried outside the PREFIX PATH first.
if (NOT OC_SYSTEM_PASTIX)
     set(OC_SYSTEM_PASTIX NO) # set it to NO so that we have a value if none is set at all (debug output)
     find_package(PASTIX ${PASTIX_FIND_VERSION} CONFIG
        PATHS ${CMAKE_PREFIX_PATH}
        QUIET
        NO_DEFAULT_PATH)
    if (PASTIX_FOUND)
        set(PASTIX_FOUND YES)
        my_stupid_package_dependent_message_function_pastix("Found version ${PASTIX_FIND_VERSION} at ${PASTIX_DIR} in CONFIG mode")
    endif()
else()
    # If local lookup is enabled, try to look for packages in old-fashioned module mode and then config modes 
    
    my_stupid_package_dependent_message_function_pastix("System search enabled")
    ...
    my_stupid_package_dependent_message_function_pastix("Trying to find version ${PASTIX_FIND_VERSION} on system in MODULE mode")
    my_stupid_package_dependent_message_function_debug_pastix("CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\nCMAKE_SYSTEM_PREFIX_PATH=${CMAKE_SYSTEM_PREFIX_PATH}\nPATH=$ENV{PATH}\nLD_LIBRARY_PATH=$ENV{LD_LIBRARY_PATH}")
    
    # Temporarily disable the required flag (if set from outside)
    SET(_PKG_REQ_OLD ${PASTIX_FIND_REQUIRED})
    UNSET(PASTIX_FIND_REQUIRED)
    ...
    #message(STATUS "find_package(PASTIX ${PASTIX_FIND_VERSION} MODULE QUIET)")
    find_package(PASTIX ${PASTIX_FIND_VERSION} MODULE QUIET)
    
    # Restore stuff
    SET(PASTIX_FIND_REQUIRED ${_PKG_REQ_OLD})
    if (_readd)
    ...
    if (PASTIX_FOUND)
        ...
        set(PASTIX_FOUND YES)
        if (NOT TARGET pastix)
            set(LIBS ${PASTIX_LIBRARIES})
            my_stupid_package_dependent_message_function_pastix("Found: ${LIBS}")
            
            SET(INCS )
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED PASTIX${DIRSUFF})
                    LIST(APPEND INCS ${PASTIX${DIRSUFF}})
                endif()
            endforeach()
            my_stupid_package_dependent_message_function_debug_pastix("Include directories: ${INCS}")
            
            my_stupid_package_dependent_message_function_debug_pastix("Converting found module to imported targets")
            ...
            my_stupid_package_dependent_message_function_debug_pastix("Current build type: CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -- CURRENT_BUILD_TYPE=${CURRENT_BUILD_TYPE}")
            
            list(GET LIBS 0 _FIRST_LIB)
            add_library(pastix UNKNOWN IMPORTED)
            ...
            foreach(LIB ${LIBS})
                my_stupid_package_dependent_message_function_debug_pastix("Adding extra library ${LIB} to link interface")
                append_link_library(pastix ${LIB})
            endforeach()
        else()
            my_stupid_package_dependent_message_function_pastix("Avoiding double import of target 'pastix'")
        endif()
    else()
        my_stupid_package_dependent_message_function_pastix("Trying to find version ${PASTIX_FIND_VERSION} on system in CONFIG mode")
        
        # First look outside the prefix path
        my_stupid_package_dependent_message_function_debug_pastix("Calling find_package(PASTIX ${PASTIX_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)")
        find_package(PASTIX ${PASTIX_FIND_VERSION} CONFIG QUIET NO_CMAKE_PATH)
        
        # If not found, look also at the prefix path
        if (PASTIX_FOUND)
            set(PASTIX_FOUND ${PASTIX_FOUND})
            my_stupid_package_dependent_message_function_pastix("Found at ${PASTIX_DIR} in CONFIG mode")
        else()
            my_stupid_package_dependent_message_function_pastix("No system package found/available.")
            find_package(PASTIX ${PASTIX_FIND_VERSION} CONFIG
                QUIET
                PATHS ${CMAKE_PREFIX_PATH}
...
if (PASTIX_FIND_REQUIRED AND NOT PASTIX_FOUND)
    message(FATAL_ERROR "OpenCMISS FindModuleWrapper error!\n"
        "Could not find PASTIX ${PASTIX_FIND_VERSION} with either MODULE or CONFIG mode.\n"
        "CMAKE_MODULE_PATH: ${CMAKE_MODULE_PATH}\n"
        "CMAKE_PREFIX_PATH: ${CMAKE_PREFIX_PATH}\n"
        "Allow system PASTIX: ${OC_SYSTEM_PASTIX}\n"
        "Please check your OpenCMISSLocalConfig file and ensure to set USE_PASTIX=YES\n"
