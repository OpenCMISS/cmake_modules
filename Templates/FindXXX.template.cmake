        ...
        if (NOT TARGET @PACKAGE_TARGET@)
            ...
            foreach(DIRSUFF _INCLUDE_DIRS _INCLUDES _INCLUDE_PATH _INCLUDE_DIR)
                if (DEFINED @PACKAGE_NAME@${DIRSUFF})
                    foreach(FDIR ${@PACKAGE_NAME@${DIRSUFF}})
                       if( EXISTS ${FDIR} )
            ...
            if (_OPTIMIZED_CONFIG_INDEX EQUAL -1 AND _DEBUG_CONFIG_INDEX EQUAL -1)
                @DEBUG_MESSAGE@("Index: _OPTIMIZED_CONFIG_INDEX=${_OPTIMIZED_CONFIG_INDEX}, _DEBUG_CONFIG_INDEX=${_DEBUG_CONFIG_INDEX}")
                add_configuration_link_libraries(@PACKAGE_TARGET@ ${CURRENT_BUILD_TYPE} "${LIBS}")
            else()
                @DEBUG_MESSAGE@("Index: _OPTIMIZED_CONFIG_INDEX=${_OPTIMIZED_CONFIG_INDEX}, _DEBUG_CONFIG_INDEX=${_DEBUG_CONFIG_INDEX}")
                ...
                if (_OPTIMIZED_CONFIG_INDEX GREATER -1)
                    ...
                    add_configuration_link_libraries(@PACKAGE_TARGET@ RELEASE "${_EXTRACTED_LIBS}")
                endif()
                if (_DEBUG_CONFIG_INDEX GREATER -1)
                    extract_config_libs(${_DEBUG_CONFIG_INDEX} "${LIBS}" _EXTRACTED_LIBS)
                    add_configuration_link_libraries(@PACKAGE_TARGET@ DEBUG "${_EXTRACTED_LIBS}")
                endif()
            endif()
        else()
...
if (@PACKAGE_CASENAME@_FIND_REQUIRED AND NOT @PACKAGE_CASENAME@_FOUND)
    message(FATAL_ERROR "FindModuleWrapper error!\n"
        ...
        "Allow system search for @PACKAGE_NAME@: ${@PACKAGE_NAME@_FIND_SYSTEM}\n"
